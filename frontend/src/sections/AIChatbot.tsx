...
import { X, Send, Bot, Sparkles, HelpCircle, Calendar, Home, CreditCard } from 'lucide-react';
...
const quickActions = [
  { icon: Calendar, label: 'Boka städning', action: 'boka' },
  { icon: Home, label: 'Våra tjänster', action: 'tjanster' },
  { icon: CreditCard, label: 'RUT-avdrag', action: 'rut' },
  { icon: HelpCircle, label: 'Priser', action: 'priser' },
];

// Comprehensive FAQ and responses
const getBotResponses = (): Record<string, { text: string; suggestions?: { label: string; action: string }[] }> => ({
  default: {
...
// Map user input keywords to response keys
...
export function AIChatbot({ onBookNow }: AIChatbotProps) {
  ...
  useEffect(() => {
    if (messages.length === 0) {
      addBotMessage(botResponses.default.text, botResponses.default.suggestions);
    }
  }, []);
  ...
  const addBotMessage = (text: string, suggestions?: { label: string; action: string }[]) => {
    setIsTyping(true);
    setTimeout(() => {
      setMessages((prev) => [
        ...prev,
  ...
  const handleSend = () => {
    ...
    const responseKey = getResponseKey(inputValue);
    const response = botResponses[responseKey] || botResponses.hjalp;
  ...
  const handleQuickAction = (action: string) => {
    if (action === 'book_now') {
      if (onBookNow) {
        ...
        addBotMessage('Klicka på "Boka nu"-knappen i menyn för att boka din städning!', [
          { label: 'Tillbaka', action: 'default' },
        ]);
      }
      return;
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-50 flex items-center gap-2 bg-teal-500 hover:bg-teal-600 text-white px-4 py-3 rounded-full shadow-lg transition-all hover:scale-105 ${
          isOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'
        }`}
      >
        <Sparkles className="w-5 h-5" />
        <span className="font-medium">Fråga vår AI</span>
      </button>

      {/* Chat Window */}
      <div
        className={`fixed bottom-6 right-6 z-50 w-96 max-w-[calc(100vw-3rem)] bg-white rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 ${
          isOpen
      ...
      >
        ...
        <div className="bg-gradient-to-r from-teal-500 to-teal-600 p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-teal-100">Alltid här för att hjälpa</p>
            </div>
          </div>
          <button
        ...
        {/* Quick Actions */}
        <div className="p-3 bg-slate-50 border-b flex gap-2 overflow-x-auto">
          {quickActions.map((action) => (
            <button
              key={action.action}
              onClick={() => handleQuickAction(action.action)}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 rounded-full text-sm whitespace-nowrap hover:border-teal-300 hover:text-teal-600 transition-colors"
            >
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    {message.suggestions.map((suggestion) => (
                      <button
                        ...
                        className="px-3 py-1 bg-white/80 hover:bg-white text-xs rounded-full transition-colors border border-slate-200"
                      >
                        {suggestion.label}
                      </button>
                    ))}
